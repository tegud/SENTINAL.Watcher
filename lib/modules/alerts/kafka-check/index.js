...
module.exports = function () {
	...
	function check() {
		...
		async.parallel([
			function (callback) {
				topicChecker(consumerCounter, zkConnect, 'healthcheck', 3, function (result) {
					var eventInfo = {
						level: "info",
			...
			function (callback) {
				topicChecker(consumerCounter, zkConnect, 'replicated_healthcheck', 1, function (result) {
					var eventInfo = {
						level: "info",
	...
	function kafka_monitor(callback) {
		...
		kafkaMonitor.on('TOPIC_HAS_NO_LEADER', function (topic) {
			...
			topicsWithNoLeaderData.topicWithNoLeaders.push(topicLeaderData);
		...
		kafkaMonitor.on('TOPIC_STATE_DATA_CHANGED', function (data) {
			...
			topicStateData.topicStates.push(data);
			events.emit(baseEventName + '.topicStates', topicStateData);
		});
...
// 					kafkaOffsets(kafkaClient, function (err, data) {
// 						offsets = data;
// 						debug('offsets', data);
// 						callback(err);
// 					});
...
// //							var topicOffsets = _.find(offsets, topic.name) 
...
// 		}, 10000);
	...
	function init_kafka_client(callback) {
		...
		kafkaClient.on('ready', function () {
			...
			debug('kafkaClient ready : ' + kafkaClient);
			callback();
		});
	};
