var zookeeper = require('node-zookeeper-client'),
    ...
    util = require('util'),
    async = require('async'),
    debug = require('debug')('sentinel:alerts:kafka-check:ZKClientWrapper'),
    client,
    self;
...
ZKClientWrapper.prototype.monitorZkPath = function(zkPath, eventNameToFire) {
    ...
    async.whilst(
        ...
        function(err){
            debug('Monitoring stopped on ZK PATH : ' + zkPath + ' (Event name : ' + eventNameToFire + ') reason : ' + err);
        }
    );
}
...
function monitorPath(zkPath, callback){
    client.getChildren(zkPath,
        ...
        function(err, children, stat) {
            if (err) {
                debug('monitorPath (' + zkPath + ') error : ' + err);
                callback(err);
            }
        }
    );
...
ZKClientWrapper.prototype.getZkPathChildren = function(zkPath, callback) {
    client.getChildren(zkPath, function(err, data, stat) {
        if(err)
        {
            debug('getZkPathChildren (' + zkPath + ') error : ' + err);
            return callback(err);
        }
...
ZKClientWrapper.prototype.monitorZkNode = function(zkPathToNode, eventNameToFire) {
    ...
    async.whilst(
        ...
        function(err){
            debug('Monitoring stopped on ZK NODE : ' + zkPathToNode + ' (Event name : ' + eventNameToFire + ') reason : ' + err);
        }
    );
}
...
function monitorNode(zkPathToNode, callback){
    client.getData(zkPathToNode,
        ...
        function(err, children, stat) {
            if (err) {
                debug('monitorNode (' + zkPathToNode + ') error : ' + err);
                callback(err);
            }
        }
    );
...
ZKClientWrapper.prototype.getZkPathData = function(zkPath, callback) {
    client.getData(zkPath, function(err, data, stat) {
        if(err)
        {
            debug('getZkPathData (' + zkPath + ') error : ' + err);
            return callback(err);
        }
