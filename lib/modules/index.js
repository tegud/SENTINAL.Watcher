...
module.exports = {
	initialiseAll: function(callback) {
		...
		var moduleTypeTasks = _.map(allModuleTypes, function(moduleType) {
			var moduleTasks = _.map(moduleType.modules, function(module) {
				return require('./' + moduleType.name + '/' + module).initialise || function(callback) {
					callback();
				};
			});

			console.log('Initialising ' + moduleType.name + ', ' + moduleType.modules.length + ' item(s) to initialise.')
