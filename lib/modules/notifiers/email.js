...
module.exports = function() {
	...
	function buildEmailOptions(event, notifierConfig, callback) {
		...
		if(notifierConfig.subject.indexOf('{{') > -1) {
			templating.renderFromTemplateString(notifierConfig.subject, event, function(err, templatedSubject) {
				emailOptions.subject = templatedSubject;
	...
	return {
		configure: function(config, callback) {
			...
			console.log(`Reading templates from directory ${templateDir}`);
