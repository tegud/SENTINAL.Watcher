...
function query() {
	...
	var self = {
		...
		addAggregation: function(config) {
			objectUtils.setDeepProperty(baseQuery, 'aggs', {});

			var name = Object.keys(config)[0];

			baseQuery.aggs[name] = config[name];
		},
...
module.exports = function(options) {
	...
	if(options.aggregations) {
		_.each(options.aggregations, function(aggregation) {
			builtQuery.addAggregation(aggregation);
		});
	}
